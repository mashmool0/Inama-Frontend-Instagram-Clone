...
import { AtSign, ImageIcon, Sparkles, User } from 'lucide-react'

import { Button } from '@/components/ui/Button'
import { Input } from '@/components/ui/Input'
import { clearPendingRegistration, getPendingRegistration } from '@/lib/session'
import { useSetUsernameMutation } from '../hooks'
...
export function SetUsernamePage() {
  ...
  const [displayName] = useState(pendingRegistration?.displayName || '')
      ...
      <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: 'spring', duration: 0.8 }} className="w-full max-w-md relative z-10">
        <div className="relative bg-card/80 backdrop-blur-xl border border-border/50 rounded-3xl shadow-2xl shadow-primary/10 p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold mb-2">تکمیل پروفایل</h1>
            <p className="text-muted-foreground">نام کاربری شما برای نمایش عمومی استفاده می‌شود</p>
          </div>
          ...
          >
            <div className="relative">
              <Input value={displayName} disabled className="pr-12" />
              <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
            </div>
            <div className="relative">
              <Input value={username} onChange={(event) => setUsername(event.target.value)} className="pr-12" placeholder="نام کاربری" required />
              <AtSign className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
            </div>
            <Button type="submit" size="lg" className="w-full">
              {setUsernameMutation.isPending ? 'در حال ذخیره...' : 'ورود به اینما'}
            </Button>
          </form>
